...
SELECT FirstName + ' ' + LastName AS FullName, a.Country, a.ZIP, '$' + CAST(MAX(cig.PriceForSingleCigar) AS NVARCHAR) AS CigarPrice
	FROM Clients AS c
JOIN Addresses AS a ON c.AddressId = a.Id
JOIN ClientsCigars AS cc ON cc.ClientId = c.Id
JOIN Cigars AS cig ON cig.Id = cc.CigarId
WHERE ZIP LIKE '%[0-9]%'
AND ZIP NOT LIKE '%[A-Z]%'
GROUP BY c.FirstName, c.LastName, a.Country, a.ZIP

-- 10. Cigars by Size

SELECT LastName, AVG(s.[Length]) AS CiagrLength, CEILING(AVG(s.RingRange)) FROM Clients AS c
	JOIN ClientsCigars AS cc ON cc.ClientId = c.Id
...
AS
BEGIN
	SELECT CigarName, '$' + CAST(PriceForSingleCigar AS VARCHAR), t.TasteType, b.BrandName,
